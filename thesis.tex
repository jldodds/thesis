...
Canonical form, on the other hand, has many of the same benifits
as semi-canonical form. It conveniently organizes the three types of
assertions and hides conjunctions in the same way Semi-canonical form
does. It is slightly less convenient to write assertions in canonical
form though, because every variable that will be referred to in a
program must be explicitly mentioned in the $\LOCAL$ and also
explicitly given a name there. When moving through a program, though,
the automation will always be able to use these names and never be in
a position to come up with names on its own. This can be a huge help
to the proof writer when keeping names straight. It will also perform
better while stepping through many statements by replacing an
autorewrite operation with simplification of a computation over an
efficient map datastructure. The computational dastructure is hard for
a human reader to understand, however, and further research is needed
to understand how to most efficiently display the information held in
the map without losing out on the performance advantages.
