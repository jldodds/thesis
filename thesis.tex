...
If we wish to use the tactic we proved to help solve another goal
...
Now we can create a deep embedding, along with a denotation function
for nat, addition, and equality
...
but we can compute on it inside of a Gallina program. When we apply
...
This isn't useful in proving the \emph{reification} (deep embedding
that is equivalent to a shallow embedding---making the goal ``real'')
of our earlier goal though:
