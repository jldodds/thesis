...
The environment is still passed to the $\SEP$, however, which still
means we can't do syntactic substitution. To fix this, we simply no
longer pass the environment to the $\SEP$. That's not to say that the
$\SEP$ has no way to talk about local variables --- it would be
useless if it couldn't. It can still reference local variables by
sharing a Coq variable with the $\LOCAL$. To restrict the $\SEP$ we
introduce a function that should be at the top level of any canonical
form assertion:
...
All that assertD does is change the type of \lstinline|R| from 
\lstinline|list (environ -> mpred)| to \lstinline|list mpred|.
So if we had a semi-canonical assertion:
...
With these two changes we get \emph{canonical form.} We have defined
\lstinline|assertD|, which gives a syntactic restriction on using the
environment in the separation part of the assertion, now we describe
how to syntactically restrict the $\LOCAL$.  
Let \lstinline{$T_1$: PTree val} 
be an efficient computational map from C program
...
is a list of assertions about the contents of the \lstinline{environ},
the non-memory portion of the program state; we do not need
\emph{arbitrary} assertions of type \lstinline{list(environ->Prop)}.
...
\lstinline|rforward|, or reified forward, we quickly get two proof goals back:
...
Which is the entailment that must be solved to show that the
...
OCaml. \footnote{We use an OCaml plugin because Ltac typechecks
  aggressively when creating terms, leading to quadratic-time behavior
...
cancellers. Finally, \lstinline|func'| is our syntax definition.
...
denotation function see see \cite[section~3.1]{malecha:thesis}
...
unfolded. To see what the problem is, we examine a term that will
explode when \lstinline|vm_compute| is run on it. For example:
...
Goal forall n, do_n (fun n => if (ltb 1 n) then  n else n)  50 n = n.
vm_compute.
\end{lstlisting}
...
There is no problem in reifying a complicated term like this, giving
us a term that looks something like 
\lstinline|exprD (complex_thing_reified)|. 
If we run \lstinline|vm_compute| on this
term to try to get the denotation, however, the complex thing will also be
unfolded once the denotation is taken, resulting in a likely massive term
that can take a very long time to compute. The solution is to
...
whitelist of functions that it should efficiently unfold.
...
\begin{itemize}
\item Line 5: There is a type \lstinline|ty| for \lstinline|id| in the
  type context
...
This is the real tactic, with much of the middle omitted:
...
case where \lstinline|as_tree| returns \lstinline|None|. This doesn't
mean that the function doesn't typecheck because there is no
requirement on expressions that a PTree can only be represented by
those two constructors. There could be another constructor with the
same denotation, or more likely a PTree could be an application of
\lstinline|PTree.set| that didn't get simplified. The following is the
